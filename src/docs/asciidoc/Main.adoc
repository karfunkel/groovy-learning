...
Eine Zeichenkette (String) kann mit `Date.parse("dd.MM.yyyy", datum)` in ein `Date`-Objekt gewandelt werden.			/*evtl. Beispiel bzw. Erklärung was ein Date-Objekt ist*/
====
...
Eine Zeichenkette (String) kann mit `input.split(',')*.toInteger()` in eine Liste aus Ganzzahlen umgewandelt werden.					/*.toInteger muss früher angegeben und besser erklärt werden auch .split*/
====
